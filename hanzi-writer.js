...
var HanziWriter = (function () {
  ...
  const globalObj = typeof window === 'undefined' ? global : window; // 错误笔画信息
  ...
  class Quiz {
    ...
    endUserStroke() {
      if (!this._userStroke) return; // 当用户画一笔，就直接删除
      ...
      const currentStroke = this._getCurrentStroke(); // 检测是否跟数据中的point是否匹配。
    ...
    _completeOperation() {
      ...
      console.log('是否已经完成', this._currentStrokeIndex, strokes.length);
      let animation = showStroke('main', this._currentStrokeIndex, strokeFadeDuration); // 是否已经完成
      ...
      if (isComplete) {
        ...
        showHighlight && this._renderState.run(animation);
      }
    }
    ...
    _handleSuccess(meta) {
      ...
      const animation = showStroke('main', this._currentStrokeIndex, strokeFadeDuration);
      this._mistakesOnStroke = 0;
      this._currentStrokeIndex += 1; // 当完成笔画后执行，这个方法里有判断是否完成笔画

      this._completeOperation(); // 执行动画
    ...
    _handleFailure(meta) {
      ...
      const {
        mistakeList,
        mistakeCurEl
      } = mistakeInfo; // 标记当前错误笔画

      mistakeList.push(mistakeCurEl);
      this._mistakesOnStroke += 1;
      this._totalMistakes += 1; // 不管正确与否都+1
  ...
  class UserStrokeRenderer {
    ...
    mount(target) {
      this._path = createElm('path'); // 记录当前错误的元素

      mistakeInfo.mistakeCurEl = this._path;
